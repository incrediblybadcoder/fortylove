...
import ch.fortylove.views.newgrid.rows.CourtComponent;
import ch.fortylove.views.newgrid.rows.OverviewHeaderComponent;
...
@Component
public class OverviewGridComponent extends VerticalLayout {

    @Nonnull final private CourtService courtService;
    @Nonnull final private BookingSettingsService bookingSettingsService;

    @Autowired
    public OverviewGridComponent(@Nonnull final CourtService courtService,
...
//        setSpacing(false);
    ...
    public void build(@Nonnull final LocalDate date) {
        ...
        add(new OverviewHeaderComponent(bookingSettings));

        // courts
        final List<Court> courts = courtService.findAll();
        courts.forEach(court -> add(new CourtComponent(bookingSettings, court)));
